...
python main.py cache-examples data/datasets/drop/train.json \
...
python -m torch.distributed.launch --nproc_per_node=4 --use_env main.py \
  ...
  --model-path "$MODEL" --model-type "$MODEL_TYPE" --eval-file data/datasets/drop/dev.json \
  ...
  --fp16 --debug-features --save-steps 0 --num-train-epochs 18 --num-workers 8
