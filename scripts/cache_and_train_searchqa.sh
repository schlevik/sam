...
python main.py cache-examples data/datasets/searchqa/train.json \
...
python -m torch.distributed.launch --nproc_per_node=4 --use_env main.py \
  ...
  --model-path "$MODEL" --model-type "$MODEL_TYPE" --eval-file data/datasets/searchqa/dev.json \
  ...
  --fp16 --debug-features --save-steps 0 --num-train-epochs 2 --num-workers 8
