...
sentences = {
    "goal": [
        ...
        "$ACTORTEAM.name-pos-pre player $ACTOR $VP.attention %CONNECTIVE.ADVP , @MODIFIER.RB.goal "
        "$VBG.goal a ($JJ.positive) goal $DISTANCE.PP .",

        "$ACTOR 's goal ( , [$RDM.VBG.goal|$RDM.NOVB] , ) @MODIFIER.RB.goal arrived $TIME after !PRPS teammate "
        ...
        "$COREF-PLAYER swept $POSITION.HEIGHT.NN to the $POSITION.BOX for $ACTOR to @MODIFIER.RB.goal "
        "poke past the $GOALKEEPER .",

        "A $JJ.positive $DISTANCE.JJ strike from $ACTOR @MODIFIER.RB.goal [flying|homing] into $POSITION.GOAL past "
        ...
        "@MODIFIER.RB.goal $VBDO.goal (just) $POSITION.PP.GOAL (to leave the $GOALKEEPER with no chance) .",
        ...
        "%CONNECTIVE.VBD> $ACTOR , on the end of it , @MODIFIER.RB.goal $VBDO.goal $POSITION.GOAL $RDM.VBG.goal .",
